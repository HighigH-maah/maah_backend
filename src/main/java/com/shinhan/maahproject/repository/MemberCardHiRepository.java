...
//			+ "AND ch.cardHistoryDate <= :endDate "
			) // Include the missing column in the GROUP BY clause
...
//			Integer sumHiCardTotalLimitByMemberBYOwner (@Param("memberId") MemberVO member, @Param("connectHiCard") MemberCardHiVO connectHiCard);
	...
	@Query("SELECT SUM(mb.memberByLimit) FROM MemberCardByVO mb " + "WHERE mb.member = :memberId "
			+ "AND mb.connectHiCard = :connectHiCard")
	Integer sumHiCardTotalLimitByMemberBYOwner(@Param("memberId") MemberVO member,
			@Param("connectHiCard") MemberCardHiVO connectHiCard);
	...
	@Modifying
	@Query("UPDATE MemberCardHiVO mch " + "SET mch.memberAccountKey.memberAccountNumber = :#{#accch.bankName}, "
